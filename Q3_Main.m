...
if plot_transition_probabilities
    % Create a new figure with a specific name
    figure('Name', 'Transition Probabilities', 'Color', 'w'); % White background
    hold on;

    % Plot numerical transition probabilities
    plot(time / pi, transition_probabilities, 'LineWidth', 2.9, 'Color', [0, 0.447, 0.741], 'LineStyle', '-'); % Blue solid

    % Plot theoretical transition probabilities if enabled
    if plot_theory
        plot(time / pi, transition_prob_theory, 'LineWidth', 2.9, 'Color', [0.850, 0.325, 0.098], 'LineStyle', '-'); % Orange dash-dot
    end

    % Customize axes
    xlim([min(time / pi), max(time / pi)]);
    ylim([0, 1.3 * max([max(transition_probabilities), max(transition_prob_theory)])]); % Dynamic y-limits
...
ylabel('Transition Probability $P_{1 \leftarrow 0}$', 'FontSize', fs, 'Interpreter', 'latex');
    ...
    % Add legend
    legend({'Numerical (Split-Operator)', 'Theoretical (TDPT)'}, 'FontSize', fs, ...
        'Location', 'northeast', 'Box', 'on'); % Dynamic legend

    % Add title
    %title('Transition Probability to First Excited State', 'FontSize', fs, 'FontWeight', 'normal');

    % Optimize ticks
    xticks([0, 10, 20, 30, 40, 50, 60]); % Fewer ticks
    yticks([0, 0.02, 0.04, 0.06, 0.08]); % Fewer ticks
    set(gca, 'FontSize', fs, 'LineWidth', 1.2);

    % Save the figure
    pbaspect([2 1 1]); % Set aspect ratio
    exportgraphics(gcf, 'Transition_Probabilities_Styled.png', 'Resolution', 300);
