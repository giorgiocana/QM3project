...
%   trap using fast Fourier transform (FFT) with varying amplitude (A)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A_values = [0.01, 0.05, 0.1, 0.15];
w = 1.1;                      % Fixed omega
w0 = 1.0;                      % Natural frequency

% Initialize variables for plots
fs = 22;                       % Font size for plots
transition_probabilities_all = zeros(length(A_values), M); % Preallocate for all amplitudes
cmap = [
...
for idx = 1:length(A_values)
    A = A_values(idx);         % Current amplitude
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Plotting the transition probabilities for different amplitudes (A)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

figure('Name', 'Transition Probabilities vs. Amplitude', 'Color', 'w');
hold on;
...
for idx = 1:length(A_values)
    ...
    plot(time_downsampled, probabilities_downsampled, 'LineWidth', 2.9, ...
         'Color', cmap(idx, :), ...
         'DisplayName', sprintf('A = %.2f', A_values(idx)));
end
...
% Save the plot
exportgraphics(gcf, 'Transition_Probabilities_vs_Amplitude.png', 'Resolution', 300);
