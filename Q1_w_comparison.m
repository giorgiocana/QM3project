...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Numerical simulation comparing different perturbation frequencies (w)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
A = 1.0;                     % Fixed perturbation amplitude.
w_values = [1.01, 1.1, 1.5];   % Array of perturbation frequencies
...
% Store results for each w
psis = cell(1, length(w_values));

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Main simulation loop over different w values
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for idx = 1:length(w_values)
    w = w_values(idx);        % Current perturbation frequency
    ...
    % Store the final state for the current w
    psis{idx} = psi_0;
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if plot_final_state
    figure('Name', 'Final State (Comparing w)', 'Color', 'w'); % White background
    hold on;

    % Plot each psi for corresponding w with distinct styles and colors
    colors = {
        [0, 0.447, 0.741],  % Blue
    ...
    % Add legend
    legend(arrayfun(@(w) sprintf('w = %.2f', w), w_values, 'UniformOutput', false), ...
        'FontSize', fs, 'Location', 'northeast', 'Box', 'on');
    annotation('textbox', [0.241, 0.621, 0.1, 0.1], ...
               'String', sprintf('$A = %.2f, T = %.0f \\pi, M = 10^5 $', A, T / pi), ...
               'Interpreter', 'latex', ...
               'FontSize', fs, ...
    ...
    % Save the figure
    exportgraphics(gcf, 'Final_State_Comparison_w.png', 'Resolution', 300);
    hold off;
end

fprintf('Simulation comparing w completed.\n');
toc;
