...
% Set DVR parameters 
T = pi*60; %pi*pi ; %5;                  % Time duration of the evolution
...
excited_temp = (hermiteH(2,X)).*exp(-(X-X0).^2/(2*sigma^2));
...
fprintf('Vnm/A = %.4f \n', matrix_element/A);
w_diff = w - w0;
matrix_element = A^2*2e-2 ;
disp(max(transition_probabilities))

transition_prob_theory = ( (matrix_element / (w-2*w0)) * sin( (w-2*w0) * time / 2) ) .^ 2; 

amplitude = max(transition_probabilities);
transition_prob_theory = amplitude* (sin( (w-2*w0) * time / 2) ) .^ 2; 
...
if plot_transition_probabilities
    ...
    hold on;
        ...
        if plot_theory
            plot(time/pi, transition_prob_theory, 'LineWidth', 2.2, 'DisplayName', 'Theoretical (TDPT)', 'Color', 'r');
        end
    legend
    ...
    exportgraphics(gcf, sprintf('Q6_Transition_Theory_v_Numerical_A=%.3f_w=%.3f.png', A, w), 'Resolution', 300);
    hold off;
end