...
excited_temp = (hermiteH(excited_label,X)).*exp(-(X-X0).^2/(2*sigma^2));
...
for i = (1:number_of_trials) 
    ...
    psi_0 = initial_state;
    for m = 1:M
            UV = exp(-1i*( (X.^2)/2 + A*sin(w*dt*(m-1))*sin(X) )*dt/2);
...
m = excited_label;
transition_prob_theory = A^2 / exp(1/2) * cos(w * time-pi/2).^2 /(2^m * factorial(m) * m^2);
